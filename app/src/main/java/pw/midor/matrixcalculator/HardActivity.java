...
public class HardActivity extends Activity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        ...
        if(v.getId()==R.id.submitHard) {
            ...
            if(qNum==0) {
                int correct=Rank(sh,-1,4);
                if(correct==ans)
                    score+=10;
            }
            else if(qNum==1) {
                int correct = (int)(sh[0][0]* SubMatrix(0,0,sh)-sh[0][1]* SubMatrix(0,1,sh)+sh[0][2]* SubMatrix(0,2,sh)-sh[0][3]* SubMatrix(0,3,sh));
                if(correct==ans)
                    score+=10;
            }
            else if(qNum==2) {
                ...
                int correct = (int)(sh[0][0]+sh[1][1]+sh[2][2]+sh[3][3]);
                if(correct==ans)
                    score+=10;
            }
        }
